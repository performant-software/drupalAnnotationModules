		...
		*/
		...
		AnnotatorViewer.prototype.textareaEditor = function (annotator_textArea, item) {
			...
			if ($('li#sb' + item.id).find('textarea.panelTextArea').length == 0) {
				...
				var annotationCSSReference = 'li#sb' + item.id + ' > div.annotator-marginviewer-text';
				...
				$('<a href="#save" class="annotator-panel-save">Save</a>').appendTo(control_buttons).bind("click", {
					annotation: item
				}, this.onSavePanel);
				$('<a href="#cancel" class="annotator-panel-cancel">Cancel</a>').appendTo(control_buttons).bind("click", {
					annotation: item
				}, this.onCancelPanel);
			}
		};
		...
		AnnotatorViewer.prototype.onSavePanel = function (event) {
			...
			var textarea = $('li#sb' + current_annotation.id).find("#textarea-" + current_annotation.id);
			if (typeof (this.annotator.plugins.RichEditor) != 'undefined') {
		...
		AnnotatorViewer.prototype.onCancelPanel = function (event) {
			...
			if (typeof (this.annotator.plugins.RichEditor) != 'undefined') {
				...
				var anotacio_capa = '<div class="annotator-marginviewer-text"><div class="annotator-hl-' + current_annotation.category + ' anotator_color_box"></div>' + textAnnotation + '</div>';
				var textAreaEditor = $('li#sb' + current_annotation.id + ' > .annotator-marginviewer-text');

				textAreaEditor.replaceWith(anotacio_capa);
			} else {
				var textarea = $('li#sb' + current_annotation.id).find('textarea.panelTextArea');
				this.normalEditor(current_annotation, textarea);
			}
		...
		AnnotatorViewer.prototype.normalEditor = function (annotation, editableTextArea) {
			var buttons = $('li#sb' + annotation.id).find('div.annotator-textarea-controls');
		...
		AnnotatorViewer.prototype.onAnnotationUpdated = function (annotation) {
			...
			$("#sb" + annotation.id).html(this.mascaraAnnotation(annotation));
		};
		...
		AnnotatorViewer.prototype.mascaraAnnotation = function (annotation) {
			...
			var delete_icon = "<img src=\"" + IMAGE_DELETE + "\" class=\"annotator-viewer-delete\" title=\"" + i18n_dict.Delete + "\" style=\" float:right;margin-top:3px;;margin-left:3px\"/><img src=\"/sites/all/themes/sky/src/img/edit-icon.png\"   class=\"annotator-viewer-edit\" title=\"Edit\" style=\"float:right;margin-top:3px\"/>";
			...
			//Add a space between commas in tags list
		...
		AnnotatorViewer.prototype.createAnnotationPanel = function (annotation) {
			...
			var annocount = $('#anotacions-uoc-panel .annotator-marginviewer-element').length;

			var annotation_layer = '<div  class="annotations-list-uoc" style="background-color:white;"><div id="annotations-panel"><span class="rotate" title="' + i18n_dict.view_annotations + ' ' + i18n_dict.pdf_resum + '" style="padding:5px;background-color:white;position: absolute; top:10em;left: -50px; width: 155px; height: 110px;cursor:pointer">' + i18n_dict.view_annotations + '<span class="label-counter" style="padding:0.2em 0.3em;float:right" id="count-anotations">0</span></span></div><div id="anotacions-uoc-panel" style="height:80%"><ul class="container-anotacions"><li class="filter-panel">' + checboxes + '</li></ul></div></div>';
		...
		AnnotatorViewer.prototype.createReferenceAnnotation = function (annotation) {
			...
			if (annotation.id != null) {
				anotation_reference = "sb" + annotation.id;
			} else {
